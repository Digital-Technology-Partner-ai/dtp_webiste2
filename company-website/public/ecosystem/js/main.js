...
const sectionCrumb = document.getElementById('sectionCrumb');
document.getElementById('backBtn').addEventListener('click', () => closeSection());
document.getElementById('ctaBack').addEventListener('click', () => closeSection());
...
function openSection(node) {
  ...
  const d = node.dest;
  sectionKicker.textContent = `DESTINATION 0${node.index + 1}`;
  sectionTitle.textContent = d.title;
  sectionIntro.textContent = d.intro;
  sectionCrumb.textContent = `DTP // ECOSYSTEM / ${d.title}`;
  sectionBlocks.innerHTML = d.blocks.map(([h, p], i) =>
    `<div class="block"><span class="block-num">0${i + 1}</span><h3>${h}</h3><p>${p}</p></div>`
  ).join('');
  ...
  tl.to(state, { curY: tY, curX: tX, tarY: tY, tarX: tX, duration: 1.2, ease: 'power3.inOut' }, 0)
    ...
    .to(['.ui-footer', '.ui-header'], { autoAlpha: 0, duration: 0.5 }, 0.1)
    .set(sectionEl, { visibility: 'visible' }, 0.55)
    .to(sectionEl, { clipPath: 'inset(0% 0 0 0)', duration: 0.9, ease: 'power4.inOut' }, 0.55)
    .fromTo(sectionKicker, { y: 26, autoAlpha: 0 }, { y: 0, autoAlpha: 1, duration: 0.6, ease: 'power3.out' }, 1.05)
    .fromTo(sectionTitle, { yPercent: 112 }, { yPercent: 0, duration: 0.85, ease: 'power3.out' }, 1.05)
    .fromTo(sectionIntro, { y: 30, autoAlpha: 0 }, { y: 0, autoAlpha: 1, duration: 0.7, ease: 'power3.out' }, 1.2)
    .fromTo('.section .block', { y: 36, autoAlpha: 0 },
      { y: 0, autoAlpha: 1, duration: 0.65, stagger: 0.09, ease: 'power3.out' }, 1.3)
    .fromTo('.section-cta', { autoAlpha: 0 }, { autoAlpha: 1, duration: 0.6 }, 1.55);
}
...
function closeSection() {
  ...
  node.hover = 0;
  node.hoverT = 0;
  const allOps = nodes.map(n => n.uniforms.uOpacity);
  ...
  tl.to(sectionEl, { clipPath: 'inset(100% 0 0 0)', duration: 0.85, ease: 'power4.inOut' }, 0)
    .set(sectionEl, { visibility: 'hidden' })
    .to(camera.position, { z: 0, duration: 1.05, ease: 'power3.inOut' }, 0.15)
    .to(camera, {
      fov: 70, duration: 1.05, ease: 'power3.inOut',
      onUpdate: () => camera.updateProjectionMatrix(),
    }, 0.15)
    .to(ambientFade, { v: 1, duration: 0.9, ease: 'power2.out' }, 0.35)
    .to(allOps, { value: 1, duration: 0.8, ease: 'power2.out' }, 0.3)
    .to(node.uniforms.uHover, { value: 0, duration: 0.6 }, 0.3)
    .to(['.ui-footer', '.ui-header'], { autoAlpha: 1, duration: 0.6 }, 0.5);
}
...
async function boot() {
  ...
  gsap.to('#loader', { autoAlpha: 0, duration: 0.6, delay: 0.35 });

  const scales = nodes.map(n => n.mesh.scale);
  const ops = nodes.map(n => n.uniforms.uOpacity);
  gsap.to(scales, { x: 1, y: 1, z: 1, duration: 1.6, ease: 'power3.out', stagger: 0.07, delay: 0.5 });
  gsap.to(ops, { value: 1, duration: 1.2, ease: 'power2.out', stagger: 0.07, delay: 0.5 });
  gsap.to(ambientFade, { v: 1, duration: 1.8, ease: 'power2.inOut', delay: 0.8 });
  gsap.fromTo(['.ui-header', '.ui-footer'],
    { autoAlpha: 0, y: -10 },
    { autoAlpha: 1, y: 0, duration: 1, ease: 'power2.out', delay: 1.2, stagger: 0.15 });
}
boot();
